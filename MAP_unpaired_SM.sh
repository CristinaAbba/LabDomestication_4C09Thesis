...
cd ${bwa_path}
files=(${trim_path}/*_R1_unpaired.fastq.gz)

for file in ${files[@]}
do
name=${file}
base=`basename ${name} _R1_unpaired.fastq.gz`
bwa mem -t 8 -M ${ref_gen} ${trim_path}/${base}_R1_unpaired.fastq.gz ${trim_path}/${base}_R2_unpaired.fastq.gz > ${sam_path}/${base}_unpaired_aligned.sam
done
